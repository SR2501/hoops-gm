...
- **Nothing important lives only in a chat.** If it is worth returning to, it is in this repository. That is why this file exists. The task list is `docs/backlog.md`, which carries its own recounted header. A task is ready when every dependency is done. **Do not restate that count here or anywhere else** — it moved four times on 2026-08-21 alone, so a copy of it is stale on arrival and the file that owns it recounts from itself.
