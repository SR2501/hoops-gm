...
Unit 2 (the manifest disclosure contract test `data-engineer` owes under the frozen protocol: outcome-keyed field allow-list, `joined_direct_outcomes`, per-status direct-outcome counts by game date, exclusion classes by status) must land **before** any widened manifest is generated. **The go/no-go after the first season is binding and must be measured**: one season projects to only ~37 held-out `doubtful` against a floor of 30, a ~23% margin on a figure scaled from a December window that is plausibly peak `doubtful` density - if the measured count comes in under 30, stop and report rather than proceeding on the multiplier's authority. Two seasons projects to ~75, three to ~113. This lane stops at the observation layer; the fit, the calibration table and the model card are `quant`'s under the frozen protocol.
