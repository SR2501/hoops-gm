...
**41 done - 1 blocked - 74 pending - 116 total**

(Recomputed from the status markers in this finished file, never reconciled from
two headers: 116 `###` headings and 116 markers, 1:1, no duplicate
...
**2026-08-22, `data-engineer`: the ledger is now POPULATED for 2025-26, not merely buildable.** The item closed the machinery; this records that it has been run at season scale. 1,227 of 1,230 regular-season games (99.76%), 164 game dates, **43,037 rows**, 596 distinct players, in the durable database at `C:\Users\steverones\hoops-gm-data\hoops_gm.db` with its raw captures at `.\data\raw` — both outside either checkout, so a worktree removal cannot destroy them. Run as seven month slices via `injury-conversion-cohort-widening`, ~2,462 requests at the 1.1s throttle. **`schedule_import` was deliberately never run against this database**: tip-offs must come from `BoxScoreSummaryV3` during the participation pass, because `ScheduleLeagueV2` instants would degenerate the cohort manifest's `cross_source_tipoff_reconciliation` into comparing one endpoint with itself, and nothing records the provenance of a persisted instant. The observable proxy — `games with tipoff` equals `games with participation`, never 1,230 — was checked after the first slice and at the end. **Three games have no summary at source** (`0022500259`/`260`/`261`, all 2025-11-19): reproducible across two runs, and driven rather than assumed — the neighbouring `0022500258` returns a complete summary from the same endpoint on the same date, so the endpoint has not changed and those three game ids simply have no summary body while `LeagueGameFinder` lists them and `BoxScoreTraditionalV3` serves them. A cross-source disagreement, not drift; under R35 they contribute no rows and no inference is drawn from their silence. This unblocks `availability-model`'s participation-label route; its `injury-status-conversion` route remains separate.
...
### `injury-conversion-cohort-widening` - Widening the conversion cohort until the rare statuses clear the activation floor
- [ ] **pending** - Unit 1 done 2026-08-21 (prior-work review + live archive-reach probe, 30 live requests across four passes; the evidence artifact records the final 13 as observations and carries a `request_accounting` block reconciling the two). Independently reviewed by `quant` and `code-review` at exact head, which found the central vocabulary test asserted against a repository file rather than the PDF bytes, that its `DOUBTFUL` claim was never asserted and is false for 2025-26, and two more numbers wrong; all fixed. Unit 2's contract-test half done. **Go/no-go MEASURED 2026-08-21 and it CLEARS**: a 640-candidate report-only sweep of the full 2025-26 regular season against a throwaway database yields 13,819 canonical observations over all 164 game dates, and held-out `doubtful` is **84 against a floor of 30** - see `docs/adapters/nba-injury-report-2025-26-status-census.json`. Remaining: 2025-26 participation (pre-draft work), then 2024-25 and 2023-24.
- **Depends on:** `injury-conversion-cohort-population`

Sweep 2025-26, then 2024-25, then 2023-24 — newest first, so an interruption keeps the most relevant data — until every status clears `injury-status-conversion`'s 30-held-out-direct-outcome floor. Prior work and probe findings are in [`docs/models/injury-status-conversion-literature.md`](models/injury-status-conversion-literature.md).

**What the probe settled, and why it was blocking.** Secondary sources state the NBA vocabulary carries no `PROBABLE` at all; had that been true, widening would have cleared the floor for `doubtful` and left the model unactivatable on `probable` instead — a multi-hour sweep spent to stay blocked on a different status. It is **false**: `PROBABLE` and `DOUBTFUL` are both present in 2023-24 and 2024-25 live bytes. Separately, the archive reaches back to at least **2019-20**, but the pre-2023 reports use a word-spacing layout this parser refuses; the boundary is bracketed between 2023-04-05 (refused) and 2023-10-25 (parsed). So **three seasons are available and three seasons is the plan**; a fourth needs parser work, not archive access. Refusing the older layout is load-bearing rather than a limitation — those files fetch cleanly with HTTP 200, valid PDF magic and five pages of genuine data, so only the parser stands between them and a cohort of plausible nonsense, and a committed fixture pins that refusal.

**Scope, argued rather than inherited.** No minutes or games-played floor: it filters on a variable *downstream of the outcome* (the healthy-worker survivor selection arXiv 2603.26935 exists to correct), it trims `out` at 1,508 which is not scarce, and its effect on `doubtful` at 21 is unknown until the data exists. The reversible alternative is to ingest every listed player, carry a role covariate, and stratify at analysis time. Regular season only. The 2025-12-22 cadence change is a **mandatory reporting stratum**, not an optional one: legacy-era observations carry systematically longer lead times, so a pooled fit would launder lead time into the label — which is why per-season and per-era rates are reported before any pooled rate.

**Sequencing follows from a finding the original framing missed: this is a box-score ingest with an injury-report attachment.** Measured by read-only preflight against the real 2025-26 season: **1,230 games over 164 game dates, 2,462 participation requests (two per game, not one), a 45.1-minute throttle floor** — against ~670 injury-report requests for the same season. Participation dominates by 3.7x in requests and 2.0x in wall time for one season, ~5.5x/~3x across three.

**The 2025-26 participation ingest is promoted out of this item's sequence and onto the pre-draft critical path**, by coordinator decision 2026-08-21 after tracing the dependency graph: `availability-model` is blocked by two independent routes — its `injury-status-conversion` dependency *and* its own need for direct non-play labels from `player_participation` — so closing only the first does not unblock it, and everything from `expected-games` through `auction-nomination` waits on those labels regardless of what this conversion study concludes. This item was merely the first thing that would have caused that ingest to happen. Its cost is wall-clock rather than effort and cannot be compressed later.

Participation is **chunkable by date range** (`--start`/`--end`/`--limit-games`), commits per game, and upserts on `(game_id, player_id)`, so a month-sized slice is a legitimate unit. An earlier version of this item said "there is no partial-season shortcut"; that belongs to the *injury-report* backfill's `enforce_expected_game_coverage`, which is itself scoped to the requested range and carries an `allow_missing_games` escape hatch — so the real constraint is that whatever window is requested must be complete within itself. Resume is re-running the same command: per-game responses are cached for 3,650 days and the cache check precedes the throttle, so replay costs no requests and no waiting. It is **not** provably equivalent to an uninterrupted run — `LeagueGameFinder`/`PlayerGameLogs` re-fetch after 12 hours, a cached bad response replays its parse failure deterministically, and the per-game failure list does not survive across invocations.

Unit 2 (the manifest disclosure contract test `data-engineer` owes under the frozen protocol) **was moved ahead of the sweep on 2026-08-21 and the contract-test half is done**: an outcome-keyed field allow-list, scoped to every JSON published under `docs/adapters/` rather than to a manifest filename, because Unit 1 committed a non-manifest evidence artifact to that same directory and a filename-scoped test would not have looked at it. Its own mutation harness found the scanner treated lists as leaves, so an outcome breakdown added inside the probe artifact's `observations` list was undetectable; fixed and pinned. The generator half — `joined_direct_outcomes`, per-status direct-outcome counts by game date, exclusion classes by status — **is blocked by the same circularity that stopped the `--raw-root` flag**: editing `cohort_evidence.py` breaks the committed manifest's source fingerprint, and regenerating the manifest needs the database the sweep exists to fill. It lands with the regeneration.

**The go/no-go after the first season is binding and must be measured**: one season projects to only ~37 held-out `doubtful` against a floor of 30, a ~23% margin on a figure scaled from a December window that is plausibly peak `doubtful` density — and `quant` established the projection is optimistic for a second, larger reason (see the era-blending finding in the literature review's §4.1). If the measured count comes in under 30, stop and report rather than proceeding on the multiplier's authority. Two seasons projects to ~75, three to ~113. This lane stops at the observation layer; the fit, the calibration table and the model card are `quant`'s under the frozen protocol.

### `injury-status-conversion` - Modelling injury status conversion rates

- [ ] **pending** - Protocol frozen 2026-08-21 at `docs/models/injury-status-conversion-preregistration.md`; **no model is fitted and no number is emitted.** Two findings block the fit, both re-derived from the committed cohort manifest rather than from prose. **(1)** The manifest publishes canonical `status_counts` and joined `participation_outcome_counts` as two separate marginals with **no status x outcome contingency**, so no conversion rate is fittable from anything on `main`. (Row-level data for the *invalidated* v1 cohort is reachable on the local-only branch `sr2501-injury-status-conversion`, which is why the freeze carries a contamination disclosure; it is superseded and non-consumable, and the **corrected** cohort has no row-level artifact anywhere.) The corrected row-level outcomes live only in the gitignored database and raw store, which the coordinator searched for across nine worktrees plus the owner's main checkout: the one real database holds **0 rows** in both `player_participation` and `player_game_logs`. **(2)** More decisively, the activation rule requires at least 30 held-out direct outcomes for every status, and whole-cohort `doubtful` is **21**. Direct outcomes are a subset of canonical observations and a chronological holdout is a subset of the cohort, so **21 < 30 unconditionally** - activation fails on arithmetic before any outcome is examined, and `probable` at 59 would need the holdout to hold more than half the `probable` observations. **The `2025-12-08..2026-01-04` cohort can therefore never activate this model**, so regenerating it as-is would spend a full live archive sweep on a guaranteed veto. The freeze turns this into a pre-unblind admissibility gate: per-status direct-outcome counts are inputs rather than outcome values, so a cohort that cannot activate is refused *before* an unblind is spent. Resuming needs a widened cohort - see `injury-conversion-cohort-widening`, whose Unit 1 (2026-08-21) established from live bytes that all three sweepable seasons carry `probable` and `doubtful`, so widening can in principle clear the floor rather than merely moving which status blocks it.
- **Depends on:** `injury-report-ingest`, `injury-report-historical-backfill`, `injury-conversion-cohort-population`, `injury-conversion-cohort-widening`, `participation-ledger`
