...
- [x] **done** - Regenerated 2026-08-20 from corrected sources after PR #37 invalidated the 2026-08-19 artifact. The corrected bounded cohort is 173 games across 26 game dates in `2025-12-08..2026-01-04`, including `0022501229` and `0022501230` on 2025-12-13 and their 39 production logs. Every count and fingerprint was recomputed from the corrected sources; nothing was carried forward. The manifest is now the deterministic output of a committed generator (`hoops_gm.ingest.injury_report.cohort_evidence`) rather than hand-assembled, and it refuses to emit unless four independent views of the window — `LeagueGameFinder`, `PlayerGameLogs` windowed by its own `GAME_DATE`, `ScheduleLeagueV2` windowed by `gameDateTimeEst` reconciled against its UTC sibling, and persisted `nba_games` — name exactly the same games. The 2026-08-19 artifact remains preserved in history and stays non-consumable.
