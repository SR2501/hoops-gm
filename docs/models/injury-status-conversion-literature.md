...
**The census artifact behind this section is not committed, and its whole-season
total disagrees with the artifact that is.** Recovered 2026-08-25 from a branch
that never opened a pull request, this section originally cited
`nba-injury-report-2025-26-status-census.json`. That file was held back rather
than landed, because publishing a second 2025-26 canonical count beside
[`nba-injury-report-cohort-2025-10-21--2026-04-12.json`](../adapters/nba-injury-report-cohort-2025-10-21--2026-04-12.json)
with no reconciliation makes the disagreement permanent and undated. The
disagreement, stated so it can be checked:

| | census (2026-08-22, uncommitted) | committed manifest (`1912a3d`) |
|---|---:|---:|
| canonical observations | 13,819 | **13,789** |
| games | 1,230 | **1,227** |
| `out` / `questionable` / `available` | 10,478 / 1,194 / 1,491 | 10,453 / 1,191 / 1,489 |
| `doubtful` / `probable` | 221 / 435 | **221 / 435** |

The gap is 30 observations, entirely in rows with a resolved `player_id`
(unresolved is 135 in both). The likely mechanism is a **difference in selection
basis rather than a data defect**: `cohort_evidence.py:915` selects over `ready`
games only, while `in_scope` is `[*ready, *missing_tipoff]`, so the manifest
drops games with no resolved tip-off and the census did not. **That has not been
proven to account for all 30** — the shortfall localises entirely to the legacy
...
independently reproduced by the committed
[`nba-injury-report-cohort-admissibility-2025-26.json`](../adapters/nba-injury-report-cohort-admissibility-2025-26.json)
at every status, as **direct** outcomes sitting at or below each canonical bound
(`doubtful` 83 ≤ 84, `out` 2,963 ≤ 2,979, `available` 467 ≤ 469, `probable` 92,
`questionable` 335). Canonical is an upper bound on direct, so agreement in that
direction is what a consistent pair of artifacts should look like.
