...
Two agents touch this phase. `data-engineer` owns `backend/app/schedule/` (ownership matrix); `quant` consumes it for `p(play)` conditioning. The boundary between "ingested fact" and "modelling choice" was the open question Phase 1 raised and did not answer.

## Decision

`schedule-ingest` and `schedule-density` are **data-engineer's**: they emit observable facts only — `team_schedule` (date, opponent, home/away, tipoff), and `schedule_density` fields that are pure calendar arithmetic (days since last game, games in trailing N days, back-to-back flag). No judgment calls (what counts as "light," what counts as "risky") belong here.
