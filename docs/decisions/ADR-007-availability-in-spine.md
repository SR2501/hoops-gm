...
**What remains genuinely open, and is not resolved by the above.** `docs/backlog.md`'s `participation-ledger-population` was reopened from `done` to `pending` on 2026-08-31 because its own written Done clause requires "a genuine multi-season cohort," which one season does not satisfy, and because it names three games with no participation rows at all as gaps that must be explicit rather than absent — a standard it had not actually met when marked done. Separately, `docs/backlog.md`'s new `participation-opportunity-coverage` item establishes that even within the populated season, **no authoritative source exists to say whether a player's absence from a game reflects a confirmed non-play, a roster status this ledger cannot see, or a capture gap** — three players (Mike Conley, CJ Huntley, James Wiseman) show multi-game silences the ledger's own recorded 2025-11-19 outage does not explain, and no NBA-side roster-interval or transaction table exists anywhere in this schema to adjudicate why. Neither finding changes what this ADR decided; both bear on when `availability-model`, which this ADR sequences into the spine, may honestly proceed.
