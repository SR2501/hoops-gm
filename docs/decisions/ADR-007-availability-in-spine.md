...
### 2026-08-31 — factual correction: the participation ledger is populated for one season; multi-season and opportunity-complete evidence are not

**Status:** Proposed. Written by the `data-engineer` lane by architect direction, following an independent cumulative review; only the project owner accepts. This is a **factual state correction**, not a change to the Decision, Consequences, Rejected, or What-would-flip-this sections above, and does not alter this ADR's `Accepted` status or reopen the 2026-08-22 amendment's own status.

The 2026-08-22 amendment above says, in its "What this costs us" paragraph, that "the participation ledger is unpopulated." That was true when written and is stale now. As of 2026-08-31: the participation ledger holds **43,037 direct participation-outcome rows over 596 players across 1,227 of 1,230 final games for the 2025-26 regular season** at `C:\Users\steverones\hoops-gm-data\hoops_gm.db` (`docs/adapters/participation-ledger-store.md`). Direct play/non-play labels — the quantity this amendment's paragraph says the paper's remedy needs a treatment model over — exist for that one season.

**What remains genuinely open, and is not resolved by the above.** `docs/backlog.md`'s `participation-ledger-population` was reopened from `done` to `pending` on 2026-08-31 because its own written Done clause requires "a genuine multi-season cohort," which one season does not satisfy — a standard it had not actually met when marked done. **Corrected 2026-08-31, editing this still-Proposed, unmerged amendment in place rather than adding a further amendment, since it is this branch's own new text and not pre-existing ADR content:** an earlier version of this paragraph also gave "gaps that must be explicit rather than absent" as a second reason for the reopening. That is retracted, not merely narrowed: `participation-ledger-population`'s own ingest mechanism cannot manufacture an explicit row for a silently-missing player, so requiring gap-explicitness of it created a circular dependency on work its own Done clause cannot do. The reopening rests on the multi-season requirement alone. Explicit ternary classification of every at-risk player-game as confirmed-observed, confirmed-absent, or unknown belongs **solely** to `docs/backlog.md`'s `participation-opportunity-coverage`, which depends on the completed multi-season ledger and separately needs independent roster/opportunity evidence neither item currently has — three players (Mike Conley, CJ Huntley, James Wiseman) show multi-game silences the ledger's own recorded 2025-11-19 outage does not explain, and no NBA-side roster-interval or transaction table exists anywhere in this schema to adjudicate why. Neither finding changes what this ADR decided; both bear on when `availability-model`, which this ADR sequences into the spine, may honestly proceed.

**What this does not do.** It does not correct or reopen the 2026-08-22 amendment's own analysis of the load-management-paradox literature, its identification-strategy requirement, or its MAE-14.32 flip-condition bar, all of which stand as written above.
