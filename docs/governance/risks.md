...
| R32 | Fantrax `team` is `(N/A)` for 1,206 of 1,788 player rows | 🟡 | Two thirds of the payload cannot contribute team evidence to identity matching. **Absent evidence and disagreeing evidence are not the same**, and a single `confidence` float cannot express the difference — which is what a human needs when adjudicating the tail via `is_manual_override`. Resolved the Phase 1 open question in favour of building per-field match evidence | `data-engineer` |
