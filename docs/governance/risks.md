...
| R26 | **`cdn.nba.com` live feeds return 403 from this network** — the free live scorecard source is unreachable | 🔴 | Verified 2026-08-17: Akamai "Access Denied" (ref `#18.6b07d417`) on `liveData` and `staticData` JSON paths, via curl, PowerShell **and** `nba_api.live`. `www.nba.com` returns 200 from the same Akamai IP, so this is a WAF rule on the CDN JSON paths, not an IP or DNS block. **Owner must test from the home network before Phase 6.** If it fails there too, the fallback is BALLDONTLIE All-Star (~$9.99/mo), which is an owner-only decision | `data-engineer` |
