...
| Shooting-pair CHECK constraints on `projections` | `data-engineer` → `backend` | `db/models/projections.py` has `fg_volume_pair_complete` and `ft_volume_pair_complete` and **no `fg3_volume_pair_complete`**. That asymmetry is load-bearing in `api/`: a half-present made/attempted pair is one of the nine members of `projections_incomplete_evidence`, and it is drivable *only* for three-pointers, so `test_a_half_present_three_point_pair_is_refused` writes a NULL the other two pairs would reject. **Adding the missing constraint is an improvement and will break that test** with an `IntegrityError` instead of the 409 it asserts — a correct failure that needs the test rewritten against a different family member, not a constraint left off. Declared because it is a `db/models/` change that breaks an `api/` test, which nothing else would tell you |

| `lock_projection_source_scope` | `backend` → `data-engineer` | `db/lineage.py` owns it and `ingest/projections/importer.py` is its only caller, so a projection import now depends on the lineage module for locking. It lives there rather than in `ingest/` because `test_lineage_locks_are_acquired_through_exactly_one_import` pins `db/lineage.py` as the **sole** module reaching `acquire_transaction_lock` — two lock-order recorders monkeypatch that one name, and a second importer of the primitive would blind both while leaving them green. A first version of the R58 fix put the lock in `ingest/` and was caught by exactly that test. **Any future lock scope belongs in `db/lineage.py` for the same reason**, and it must go through `lock_refresh_scope`, since the same test also pins `acquire_transaction_lock` to one call site there. Ordering: this scope is last in the canonical order (settings, then NBA schedule, then per-league/derived), and `import_projection_csv` takes no other lock, so it is a leaf today |
