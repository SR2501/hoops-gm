...
## `c349` - ADJUDICATED: BOTH BRANCH-ONLY FILES ON `sr2501-injury-report-history` ARE SUPERSEDED - AND THERE IS NO `c354`

**Status:** done

2026-08-27, `data-engineer` under delegated authority. Asked to adjudicate register entry `c354`. **THERE IS NO `c354`.** The highest entry before this one is `c348`, and no entry anywhere in this file mentions the census, the branch, or the disclosure surface. The only `c354` in the repository is an incidental substring inside `backend/tests/fixtures/fantrax_getplayerids_nba.json`. The coordinator re-derived the underlying facts itself and they were right, so this is a citation defect rather than a substantive one - but it is the same shape as the ADR-006 mis-citation in the preregistration: a reference adopted without re-deriving it. AN ENTRY ID IS CHECKABLE IN TEN SECONDS AND WAS NOT CHECKED.

**THE DIFF WAS TAKEN AGAINST THE WRONG BASE.** `origin/main` against the branch tip shows ~200 files differing, nearly all `D` - on main, absent from the branch - because THE BRANCH IS BEHIND, not because it deleted anything. The branch's own delta is `git diff --name-status $(git merge-base origin/main <branch>) <branch>`, which is **14 files**. Distance re-measured: `git rev-list --count <branch>..origin/main` is **64**. The backlog item `cohort-canonical-count-reconciliation` records "188 commits behind"; that does not reproduce under rev-list, total-commit or symmetric-difference counts, and I could not find a measurement that yields 188.

### Verdict 1 - the census JSON is superseded, and was deliberately held back

`docs/adapters/nba-injury-report-2025-26-status-census.json`. "Absent from `main`" is true and loses the fact that matters: it was **held back on purpose**. `docs/models/injury-status-conversion-literature.md` §4.2 says so in terms and publishes the disagreement table, and `docs/backlog.md` carries an OPEN item `cohort-canonical-count-reconciliation` recording every number. I re-derived the numbers from the two artifacts before reading either document, and they match exactly: census **13,819** canonical observations against the manifest's **13,789**; games **1,230** against **1,227**; `out`/`questionable`/`available` 10,478/1,194/1,491 against 10,453/1,191/1,489; `doubtful` and `probable` identical at 221/435.

**CHECKABLE IN NINETY SECONDS.** Open both JSONs. The census's `activation_floor_check.held_out_status_counts` dominates `nba-injury-report-cohort-admissibility-2025-26.json`'s `section_2_admissibility.held_out_direct_outcomes_by_status` at every status - available 469>=467, doubtful 84>=83, out 2,979>=2,963, probable 92=92, questionable 335=335 - over an identical window, both artifacts declaring 2025-10-21..2026-04-12, 164 game dates, holdout 2026-03-02..2026-04-12. The preregistration §2 lines 160-174 make the gate's unit **direct outcomes** and explicitly reject canonical observations as the looser bound. The census's own `verdict_is_an_upper_bound: true` agrees. **An upper bound on a quantity whose exact value is published, for the same cohort over the same window, informs no decision the exact value does not.** It can only ever prove a cohort inadmissible; this one is admissible, measured, at 2.77x on the binding status.

### Verdict 2 - `test_disclosure_surface.py` is superseded, proven by mutation

The coordinator asked that "not superseded" be treated as an unverified claim. It is false. The successor is not `test_injury_report_archive_reach.py`, which is where the coordinator looked, but `test_cohort_admissibility.py::TestTheDisclosureSurfaceIsClosed` together with `TestOutcomeKeyedDetection` - STRICTLY STRONGER ON FOUR AXES: it globs `docs/**/*.json` rather than one directory; its allow-list is keyed `(filename, dotted path)` rather than by path alone, so a permitted field cannot migrate between artifacts; list indices normalise to `[]` rather than vanishing; and detection is by intersection on mapping keys, with a test proving a subset test is evadable.

**FIVE MUTATIONS, ALL CAUGHT BY MAIN, ZERO SURVIVORS**: (1) an outcome-keyed field added to an unrelated published JSON; (2) an outcome breakdown added to a record inside the probe artifact's `observations` list - the branch file's own M3, the attack it was written for; (3) an allow-listed field deleted from the manifest; (4) the probe's `observations` emptied, which is the non-vacuity half; (5) an allow-listed field copied into a second file. Each is a one-key JSON edit, re-runnable. THE HARNESS WAS SCRATCH AND ITS RESTORE WAS VERIFIED - the unmutated baseline was re-run to green afterwards and the working tree confirmed clean, because a harness whose restore is broken scores every mutation as caught.

**AND IT WOULD NOT MERELY ADD NOTHING - IT WOULD GO RED.** Checked out verbatim onto `main` it FAILS 2 of its 7 tests. Its allow-list is stale: it does not know `seasons[].outcomes` or `seasons[].reasons` from `participation-ledger-2025-26-coverage.json`, and it asserts EXACTLY ONE artifact carries outcome-keyed fields where main legitimately has two, listed with the mechanism stated. Landing it would put two guards in disagreement about a frozen set, and the only repair available - editing its allow-list - is exactly what §2 forbids without a v3. I BRIEFLY READ THAT RED AS "THE SURFACE GREW WHILE UNENFORCED" AND WAS WRONG; the second carrier is declared in `cohort_admissibility.py` lines 159-207 with its reasoning attached.

Its one genuinely unique assertion was ALREADY RECOVERED onto main, in `test_injury_report_archive_reach.py::test_the_probe_evidence_publishes_designations_and_no_participation_outcome`, whose docstring says so: *"Recovered from a superseded sibling of the committed guard... This assertion was the only part of it not already covered."* I verified that claim by mutation M4 rather than believing the docstring.

### The rule that really was unenforced was somewhere else

The coordinator's premise - *a test owed under a frozen protocol and not running in CI is an unenforced rule* - is right, and pointed at the wrong file. §2's disclosure surface IS enforced. **The held-back decision was not.** MEASURED: commit the census into `docs/adapters/` and run every guard that could plausibly see it - `test_cohort_admissibility.py`, `test_cohort_evidence.py`, `test_injury_report_archive_reach.py` - and **133 tests pass**. Nothing at all stopped the artifact §4.2 took care to hold back.

`backend/tests/test_status_census_reconciliation.py` now does, as a **reconciliation rather than a banned filename**: it selects on the artifact's own `kind` discriminator rather than its name, compares its whole-season totals against the committed manifest, and PASSES THE MOMENT THE TWO AGREE. That permits the outcome `cohort-canonical-count-reconciliation` is open for - land the census once the 30 are attributed - and blocks only the undated-disagreement one. Verified non-vacuous against the real artifact, which it fails with the exact §4.2 figures, and the finder and reconciler are each exercised against a known non-zero case because a broken finder reports the same zero a clean tree does.

### The three "differing" files needed no reconciliation

`test_injury_report_archive_reach.py`, `injury-status-conversion-literature.md` and `nba-injury-report.md`: main is correct in all three, 242 insertions against **4** deleted lines, and all four deletions inspected - a stale backlog id (`injury-conversion-cohort-widening` renamed to `injury-conversion-cohort-population`), a missing end-of-file newline, and two lines citing the uncommitted census, replaced by the reconciliation table. Two further files the brief did not list also differ, `test_live_smoke.py` and `docs/models/README.md`; main is correct in both, the deletions being em-dash-to-hyphen normalisations plus one refinement. NOTHING ON THAT BRANCH IS UNABSORBED EXCEPT THE CENSUS, WHICH IS HELD BACK BY DECISION.

**THE BRANCH IS NOT DELETED AND MUST NOT BE.** `origin/sr2501-injury-report-history` at `dbad3b3` is the only copy of the census artifact, and `cohort-canonical-count-reconciliation` cannot be closed without it.

### Later the same night - the 30-observation gap is attributed, and the objection that blocked it was read off the wrong field

Time recovered after a coordinator time-check correction, spent on the one "could not verify" item that was unverified only for want of it.

`docs/models/injury-status-conversion-literature.md` §4.2 and `docs/backlog.md`'s `cohort-canonical-count-reconciliation` both reject the selection-basis explanation for the 30-observation gap on the same ground: the dropped games are 2026-01-08 `MIA@CHI`, *short-lead* era, while the gap is entirely legacy. **THAT IS THE WRONG FIELD.** 2026-01-08 `MIA@CHI` is `trusted_entry_cascade.unresolved_game_id_sample`, which is **unresolved player identity** - a different exclusion class that has nothing to do with tip-offs.

The games actually dropped for want of a tip-off are `0022500259`, `0022500260`, `0022500261`, named identically by two independent fields of the committed manifest (`cross_source_tipoff_reconciliation.games_without_both_instants` and `store_assembly.receipt.cross_store_nba_games_reconciliation.tipoff_utc.absent_in_participation_ledger`), and dated by a third artifact: `participation-ledger-2025-26-coverage.json` reports `games_unobserved: 3` with `unobserved_dates` all **2025-11-19**, which is before the 2025-12-22 boundary. **The three dropped games are legacy era.** The all-legacy localisation is what the hypothesis predicts, not evidence against it.

Corroborating, and explicitly not proving: 30 over 3 games is **10.0 observations per game**, an ordinary size for one game's report rows; and differencing the census's `status_counts_by_game_date` against the committed `direct_outcome_counts_by_game_date` puts 2025-11-19 **top of all 164 dates at +30**, next +17, mean 1.35.
