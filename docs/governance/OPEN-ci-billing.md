...
**The git history is verifiably clean.** Every commit on every branch was scanned on 2026-08-17 for credential patterns. The only matches are deliberately-planted test fixtures in `test_secret_scan.py`: a joke password, AWS's own published documentation example key, and bare `BEGIN PRIVATE KEY` headers with no key material. **No real credential has ever been committed**, so going public would not expose one retroactively.

> Note: this paragraph originally quoted those fixture values verbatim and the Code gate failed on it — correctly, since a scanner cannot distinguish a documentation example from a live credential and should not try. The values are described rather than reproduced. Prose about secrets is still prose the scanner reads.
